...
class _ResetPassFormState extends State<ResetPassForm> {
  ...
  void initState() {
    ...
    _resetPasswordProvider.addListener(_listenToResetPasswordProvider);
  }

  void _listenToResetPasswordProvider() {
    final state = _resetPasswordProvider.uiState;
    if (state is Success<AuthResponse>) {
      AppToasts.showSuccess(context, message: "Password changed successfully");
        ...
        children: [
          ...
          Consumer<ResetPasswordProvider>(
            builder: (context, provider, _) {
              return AppButton(
                text: "Submit",
                isLoading: provider.uiState is Loading<AuthResponse>,
                onPressed: () {
